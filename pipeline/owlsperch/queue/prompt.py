...
_MONSTER_STAT_BLOCK_RULES: list[str] = [
    ...
    *[f"- {label} -> {target}" for label, target in _MM3_LABEL_MAP],
    "",
    "A label this layout does not print (e.g. Environment, Organization,",
    "Treasure, Level Adjustment on some entries) is simply omitted -- OMIT",
    "the key, never write `null` and never carry a value over from the other",
    "layout's conventions.",
    "",
    ...
    "",
    "A grouped entry's stat blocks are often printed as a MULTI-COLUMN table:",
    "a first column of labels, then one column per creature, the creature",
    "names as the header row. Read DOWN each column: column N's cells are",
    "creature N's own stat block, and a cell that wraps onto the following",
    "line still belongs to the column it started in. That stat table is NOT",
    "also written as a `table` record -- it is the stat block itself, and the",
    'fields above already carry it; the "Tables belonging to this entity"',
    "convention below does not apply to it. A DIFFERENT table printed in the",
    "same entry (a size/damage table, a random-encounter table) is a `table`",
    ...
    '"blindsense", "damage reduction 10/evil", "spell resistance 30", "+2',
    'turn resistance", "immunity to ...", "resistance to ...", "fast healing',
    '5", "regeneration 5", and the type-traits tokens ("undead traits",',
    '"construct traits", "plant traits", "elemental traits", "incorporeal',
    'traits", "angel traits", ...). Never invent an ability entry for one of',
    ...
    "parses it: `hd.count` is the dice count (4), `die` the die size (12),",
    "`bonus` the printed modifier (omit it when there is none). A half hit",
    'die is written as a fraction: "1/2 d10 (2 hp)" gives `hd.count` 0.5.',
    "`hp` is the average hit points printed in parentheses on that same",
    "line -- both are cross-checked against `hd.text`, so a mistyped number",
    ...
    "",
    "This segment can begin or end mid-column, so its first or last",
    "paragraphs may belong to the PREVIOUS or NEXT creature's entry.",
    "Attribute every passage by the creature it actually names, never by its",
    "position in the segment, and never take a stat value from a neighbouring",
    "creature's column or a facing page: a value that does not appear in this",
    "creature's own stat block is not this creature's. If the creature's own",
    "stat block is itself truncated here, answer `needs_context` naming the",
    "adjacent segment ids rather than filling the gap from your own knowledge",
    "of the game.",
    "",
