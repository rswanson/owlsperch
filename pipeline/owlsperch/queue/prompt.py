...
_KIND_RULES: dict[str, list[str]] = {
    ...
    "class": [
        ...
        "exactly as printed -- with ONE narrow exception: COLUMN BLEED. A",
        "Special-cell token that no prose ANYWHERE in this class's own",
        "segment text describes, AND that contradicts this class's own",
        'features (the concrete case seen in this book: a stray "Bonus',
        'Feat"/"Bonus feat" token bled in from the fighter or monk\'s own',
        "Special column on a facing page -- this book gives none of the",
        "other classes bonus feats) is column bleed, not this class's own",
        "data: drop it from the cell, say so in your reply's `notes`, and",
        "do NOT invent a `class_features` entry for it. Every OTHER token",
        "is still real data and still needs its own `class_features` entry",
        "-- see the invented-value rule below. Put the table record's own",
        "id in BOTH this record's `level_table` field AND this record's own",
        "`tables` array.",
        "",
        ...
        "entry, and that entry's `name` should match the Special cell's own",
        'spelling (e.g. plural "Bonus Feats" vs. singular "Bonus feat" --',
        "match the cell's own wording, don't invent a different form).",
        "",
    ...
    "prestige_class": [
        ...
        "The Special-column cell is reproduced verbatim, comma-separated",
        "exactly as printed -- with ONE narrow exception: COLUMN BLEED. A",
        "Special-cell token that no prose ANYWHERE in this entry's own",
        "segment text describes, AND that contradicts this entry's own",
        "features (the concrete case seen in the base-class prompt rules:",
        'a stray "Bonus Feat"/"Bonus feat" token bled in from a neighboring',
        "entry's own Special column on a facing page) is column bleed, not",
        "this entry's own data: drop it from the cell, say so in your",
